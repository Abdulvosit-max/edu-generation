...
import { Presentation, Loader2, FileText, ChevronLeft, ChevronRight, Download } from "lucide-react";
...
export default function SlideGen() {
  ...
  const downloadPPT = async () => {
    ...
    for (const slide of slides) {
      ...
      if (imageUrl) {
        slideObj.addText(textContent, { x: 0.5, y: 1.2, w: "55%", h: 4.5, fontSize: 16, color: "4A5568", valign: "top", fontFace: "Arial" });
        try {
          ...
          slideObj.addImage({ data: base64Data, x: 6.2, y: 1.2, w: 3.3, h: 4.0, rounding: true });
        } catch {
          // rasm yuklanmasa matnli layoutga o'tamiz
          slideObj.addText(textContent, { x: 0.5, y: 1.2, w: "90%", h: 4.5, fontSize: 18, color: "4A5568", valign: "top", fontFace: "Arial" });
        }
      } else {
        slideObj.addText(textContent, { x: 0.5, y: 1.2, w: "90%", h: 4.5, fontSize: 18, color: "4A5568", valign: "top", fontFace: "Arial" });
      }
          ...
          {slides && (
            <div className="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-3xl border border-blue-100 dark:border-blue-800/30 flex flex-col gap-4">
              ...
              <button
                onClick={downloadPPT}
                disabled={exporting}
                className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-semibold py-3 flex items-center justify-center gap-2 rounded-xl transition-colors shadow-sm"
              >
              ...
              <>
                ...
                <div className="absolute inset-y-0 left-0 flex items-center p-4">
                  <button
                    ...
                    className="p-3 bg-white/80 dark:bg-slate-800/80 hover:bg-white dark:hover:bg-slate-800 disabled:opacity-30 rounded-full shadow-lg border border-slate-100 dark:border-slate-700 text-slate-800 dark:text-slate-200 transition-all backdrop-blur"
                  >
                    <ChevronLeft size={24} />
                  </button>
                </div>
