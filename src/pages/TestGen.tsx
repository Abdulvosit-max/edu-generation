...
import { FileText, Loader2, Download, Search, CheckCircle, XCircle, Award } from "lucide-react";
...
export default function TestGen() {
  ...
  const downloadPDF = () => {
    ...
    if (teacherName.trim()) doc.text(`Tuzuvchi ustoz: ${teacherName}`, 20, 49);
    ...
    let kxPos = 40;
    tests.forEach((test, index) => {
      if (kyPos > 270) { kyPos = 50; kxPos += 60; }
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input
                  ...
                  max="50"
                  value={testCount}
                  onChange={e => setTestCount(Math.min(50, Math.max(1, parseInt(e.target.value) || 10)))}
            ...
            <div className="flex gap-2">
              {(["easy", "medium", "hard"] as const).map(level => {
                ...
                if (difficulty === level) {
                  ...
                  else btnClass = "bg-red-100 dark:bg-red-900/50 border-red-500 text-red-700 dark:text-red-300";
                }
                return (
                  <button
                    key={level}
                ...
                {!isSubmitted ? (
                  ...
                  <div className="bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-indigo-900/20 dark:to-blue-900/20 border border-indigo-100 dark:border-indigo-800/30 p-8 rounded-3xl mt-8 shadow-sm">
                    ...
                    {analyzing ? (
                      ...
                      <div className="prose prose-lg prose-slate dark:prose-invert markdown-body max-w-none text-slate-700 dark:text-slate-300 bg-white/50 dark:bg-slate-900/50 p-6 rounded-2xl border border-indigo-100/50 dark:border-indigo-800/30">
