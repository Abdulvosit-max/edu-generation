    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 relative">
      ...
      <div className="flex-1 overflow-auto p-4 sm:p-6 pb-32 w-full max-w-4xl mx-auto flex flex-col gap-6">
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} className={`flex ${isUser ? "justify-end" : "justify-start"} group`}>
              <div 
                className={`max-w-[85%] md:max-w-[75%] rounded-2xl p-4 text-sm leading-relaxed ${
                  isUser 
                    ? "bg-blue-600 text-white rounded-tr-none self-end" 
                    : "bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-tl-none self-start"
                }`}
              >
                {!isUser && m.id !== "1" && (
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-2xl rounded-tl-none p-4 pr-8 self-start text-sm">
      ...
      <div className="p-4 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shrink-0">
        <div className="max-w-4xl mx-auto relative flex items-center">
          ...
          >
            <Send size={16} className={loading || !input.trim() ? "" : "-ml-0.5 mt-0.5"} />
          </button>
        </div>
      </div>
    </div>
